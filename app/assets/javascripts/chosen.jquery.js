    ...
    SelectParser.prototype.add_option = function(option, group_position, group_disabled) {
      if (option.nodeName.toUpperCase() === "OPTION") {
        if (option.text !== "") {
          ...
          var custom_html = option.getAttribute("data-custom_html");
          var search_html = custom_html ? $(custom_html).text() : option.innerHTML;
          this.parsed.push({
  ...
  AbstractChosen = (function() {
    ...
    AbstractChosen.prototype.result_add_option = function(option) {
      ...
      if (!option.disabled) {
        ...
        var output_html = option.custom_html ? option.custom_html : option.html;
        style = option.style.cssText !== "" ? " style=\"" + option.style + "\"" : "";
        return '<li id="' + option.dom_id + '" class="' + classes.join(' ') + '"' + style + '>' + output_html + '</li>';
      } else {
        return "";
      }
    };
...
(function() {
  ...
  Chosen = (function(_super) {
    ...
    Chosen.prototype.choice_build = function(item) {
      ...
      this.choices += 1;
      var output_html = item.custom_html ? item.custom_html : item.html;
      if (item.disabled) {
        html = '<li class="search-choice search-choice-disabled" id="' + choice_id + '"><span>' + output_html + '</span></li>';
      } else {
        html = '<li class="search-choice" id="' + choice_id + '"><span>' + output_html + '</span><a href="javascript:void(0)" class="search-choice-close" rel="' + item.array_index + '"></a></li>';
      }
    ...
    Chosen.prototype.winnow_results = function() {
      var eregex, exact_result, found, option, part, parts, regex, regexAnchor, result, result_id, results, searchText, startpos, openpos, closepos, text, zregex, _i, _j, _len, _len1, _ref;
      ...
      for (_i = 0, _len = _ref.length; _i < _len; _i++) {
        ...
        if (!option.disabled && !option.empty) {
          if (option.group) {
            ...
            result = $("#" + result_id);
            if (regex.test(option.search_html)) {
              found = true;
              results += 1;
              if (eregex.test(option.search_html)) {
                exact_result = true;
              }
            } else if (option.search_html.indexOf(" ") >= 0 || option.search_html.indexOf("[") === 0) {
              parts = option.search_html.replace(/\[|\]/g, "").split(" ");
              if (parts.length) {
            ...
            if (found) {
              if (searchText.length) {
                if(option.custom_html) {
                  startpos = option.custom_html.search(zregex);

                  // make sure the startpos is not inside a custom_html tag
                  openpos = option.custom_html.indexOf("<", startpos);
                  closepos = option.custom_html.indexOf(">", startpos);
                  if(openpos > -1 && openpos < closepos) {
                    text = option.custom_html.substr(0, startpos + searchText.length) + '</em>' + option.custom_html.substr(startpos + searchText.length);
                    text = text.substr(0, startpos) + '<em>' + text.substr(startpos);
                  } else {
                    text = option.custom_html;
                  }
                } else {
                ...
                text = option.custom_html ? option.custom_html : option.html;
              }
              result.html(text);
